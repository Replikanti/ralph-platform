...
async function generatePRDescription(
    ...
    try {
        ...
        if (validationResult.output.includes('Biome')) {
            validationSummary.push(validationResult.output.includes('✓') || validationResult.output.includes('pass') ? '✅ Biome' : '⚠️ Biome');
        }
        if (validationResult.output.includes('TSC') || validationResult.output.includes('TypeScript')) {
            validationSummary.push(validationResult.output.includes('✓') || validationResult.output.includes('0 error') ? '✅ TSC' : '⚠️ TSC');
        }
        if (validationResult.output.includes('Ruff')) {
            validationSummary.push(validationResult.output.includes('✓') || validationResult.output.includes('pass') ? '✅ Ruff' : '⚠️ Ruff');
        }
        if (validationResult.output.includes('Mypy')) {
            validationSummary.push(validationResult.output.includes('✓') || validationResult.output.includes('Success') ? '✅ Mypy' : '⚠️ Mypy');
        }
        if (validationResult.output.includes('Trivy')) {
            validationSummary.push(validationResult.output.includes('✓') || validationResult.output.includes('0 vulnerabilities') ? '✅ Trivy' : '⚠️ Trivy');
        }
