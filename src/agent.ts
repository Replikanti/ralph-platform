    ...
    try {
        ...
        if (targetState) {
            ...
            if (currentState?.id === targetState.id) {
                console.log("Issue " + issueId + " is already in state " + statusName + ", skipping update.");
            } else {
...
async function summarizeFailurePhase(task: Task, homeDir: string, errors: string): Promise<string> {
    const prompt = "You are the Post-Mortem Analyst (Claude Opus 4.5). The AI coding agent Ralph failed to complete a task. TASK: " + task.title + " ERRORS: " + errors.substring(0, 5000) + " Write a concise, human-friendly explanation.";
...
async function createPullRequest(repoUrl: string, branchName: string, title: string, body: string): Promise<string | null> {
    ...
    try {
        ...
        if (e.message?.includes('No commits between')) return null;
...
function runClaude(args: string[], cwd: string, homeDir: string, timeoutMs: number = 300000): Promise<{ stdout: string; stderr: string }> {
    return new Promise((resolve, reject) => {
        ...
        const child = spawn(CLAUDE_PATH, args, { 
            cwd,
            env: { ...process.env, HOME: homeDir, ANTHROPIC_API_KEY: process.env.ANTHROPIC_API_KEY, CI: 'true', DEBUG: 'true', TERM: 'dumb' }
        });

        if (child.stdin) child.stdin.end();
        if (!child.pid) { reject(new Error("Failed to spawn Claude CLI")); return; }
        ...
        const timeout = setTimeout(() => { child.kill('SIGKILL'); reject(new Error("Claude CLI timed out")); }, timeoutMs);

        child.on('close', (code) => {
            clearTimeout(timeout);
            if (code === 0) resolve({ stdout, stderr });
            else reject(new Error("Claude CLI exited with code " + code + ". Stderr: " + stderr.substring(0, 500)));
        });
...
async function listAvailableSkills(workDir: string): Promise<string> {
    ...
    try {
        ...
        return dirs.filter((d) => d.isDirectory()).map((d) => "- /" + d.name).join('\n');
...
async function planPhase(workDir: string, homeDir: string, task: any, availableSkills: string, previousErrors?: string) {
    let projectGuide = "";
    try { projectGuide = await fsPromises.readFile(path.join(workDir, 'CLAUDE.md'), 'utf-8'); } catch { projectGuide = "No CLAUDE.md found."; }

    const prompt = "Plan for: " + task.title + " Desc: " + task.description + " Skills: " + availableSkills + (previousErrors ? " Errors: " + previousErrors : "");
    const { stdout } = await runClaude(['-p', prompt, '--model', 'claude-opus-4-5-20251101'], workDir, homeDir);
    const planMatch = /<plan>([\s\S]*?)<\/plan>/.exec(stdout);
    return planMatch ? planMatch[1].trim() : "No plan";
}

async function executePhase(workDir: string, homeDir: string, plan: string) {
    const prompt = "Implement: " + plan + " " + SECURITY_GUARDRAILS;
    return await runClaude(['-p', prompt, '--model', 'claude-sonnet-4-5-20250929', '--allowedTools', 'Bash,Read,Edit,FileSearch,Glob'], workDir, homeDir);
}
...
async function runIteration(iteration: number, ctx: IterationContext, previousErrors: string): Promise<{ success: boolean, output?: string }> {
    ...
    const planSpan = ctx.trace.span({ name: "Planning", metadata: { iteration } });
    ...
    const execSpan = ctx.trace.span({ name: "Execution", metadata: { iteration } });
    ...
    if (check.success) {
        ...
        if (status.staged.length > 0) {
            ...
            await updateLinearIssue(ctx.task.ticketId, "In Review", "Completed. PR: " + prUrl);
        }
        return { success: true };
    }
...
export const runAgent = async (task: Task): Promise<void> => {
    return withTrace("Ralph-Task", { ticketId: task.ticketId }, async (trace: any) => {
        ...
        try {
            const sourceClaudeDir = path.join(process.env.HOME || '/home/nitramyloh', '.claude');
