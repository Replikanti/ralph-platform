# Task 015: Create WebhookController

## Objective
Replace the monolithic `app.post('/webhook')` route in `server.ts` with a typed Ts.ED controller that delegates all business logic to injected services.

## Prerequisites
- 009 (SignatureVerificationMiddleware)
- 010 (ConfigService)
...
## Reference Files
- `src/server.ts` lines 158-167 (helper functions: `isApprovalComment`, `isStateInPlanReview`)
- `src/server.ts` lines 199-349 (comment handling: `handlePlanApproval`, `handlePlanRevisionFeedback`, `handleIterationRequest`, `handleStoredPlanComment`, `handleCommentWebhook`)
- `src/server.ts` lines 351-416 (issue handling: `shouldSkipIssueUpdate`, `handleIssueWebhook`)
- `src/server.ts` lines 418-441 (main webhook route)
...
The controller must contain **ZERO business logic** beyond routing. All decisions (approval detection, state checking, job creation) are delegated to services.
...
1. **Zero external business logic**: All I/O operations go through injected services.
2. **Helper methods**: `isApprovalComment`, `isRalphComment`, `isTerminalState` are private pure methods (no I/O), acceptable in the controller.
3. **Same response format**: All `res.status(200).send(...)` responses match the current API contract.
4. **Tombstone check**: Uses `RedisProvider` directly for the tombstone read (it's a simple key check, not worth a dedicated service).

### Important: Register in Server.ts

After creating this controller, add it to `Server.ts` configuration:
...
(This wiring is done in Task 022.)

## Acceptance Criteria
- [ ] `src/controllers/WebhookController.ts` exists with `@Controller("/webhook")` decorator
- [ ] `@UseBefore(SignatureVerificationMiddleware)` applied at controller level
- [ ] Injects: QueueService, ConfigService, PlanStoreService, LinearClientService, RedisProvider
- [ ] All webhook handling logic preserved: issue create/update, comment approval/revision/iteration
- [ ] Ralph's own comment filtering preserved (prevents auto-execution)
- [ ] Tombstone check preserved
- [ ] Terminal state check preserved
- [ ] All response formats match current API contract
