...
Ralph distinguishes between two types of failures, each with a different recovery path:

1.  **Infrastructure Failures (Network, Timeouts, Crashes)**:
    - Managed by **BullMQ** via exponential backoff retries.
    - Idempotency is guaranteed by resource-based `jobId` deduplication.
2.  **Logic Failures (Linter errors, Type mismatches, Hallucinations)**:
    - Managed by an **In-Context Feedback Loop**.
